...
namespace Panosen.CodeDom.CSharp.Engine
{
    partial class CSharpCodeEngine
    {
        ...
        public void GenerateMethod(CodeMethod codeMethod, CodeWriter codeWriter, GenerateOptions options = null)
        {
            ...
            //调用其他构造函数
            if (!string.IsNullOrEmpty(codeMethod.BaseConstructor))
            {
                codeWriter.Write(Marks.WHITESPACE).Write(Marks.COLON).Write(Marks.WHITESPACE).Write(codeMethod.BaseConstructor);
            }

            codeWriter.WriteLine().Write(options.IndentString).WriteLine(Marks.LEFT_BRACE);
